...
| `barsicons.lua` | Icon/name resolvers per slot type (`ns._bar_getIcon` / `_getName` / `_getPetIcon`), consumed by the shared `ui.BarsPreview`; `_getName` (name-first: stored identity, then captured list position) feeds the preview's hover text. The richer `_addTooltip` / `_addPetTooltip` builders went dead with the interactive grid in #126 and were deleted in #132 — the preview owns its own hover text |
| `barlabels.lua` | `BAR_DEFS` (abm ↔ display label) + `ns.GetBarLabel(abm)` — names an abm bar the way the UI/overlay do (used by restore warnings, the overlay, the dupe scanner). Labels must match LibNUI's `ui.BarsPreview` row labels, which are the constrained surface (fixed label column, stacked one char per line on vertical bars) — hence `Sky`, not `Skyriding`. Bar order is now driven by real topography, so there is no user-configurable order (was `barsview_defs.lua`; `GetActiveBarOrder` dropped) |
| `barselect.lua` | `ns.BuildBarSelect(parent) → { GetChecked, AllChecked, SetAllChecked, SetOnChanged, SetHighlighter, Height }` — the per-bar **include selector**: a compact chip grid (`1‑8` / `C1‑C5` / `Bonus` / `Sky` / `Pet`) feeding the Save/Load `barFilter`. Chips default included; hovering one highlights its real bar in the preview. Modeled on Warbandeer's `BarsApply` (#124) |
...
ns.BuildBarSelect(parent)             -- → { GetChecked(), AllChecked(), SetAllChecked(v),
                                      --     SetOnChanged(fn), SetHighlighter(fn), Height() }
                                      --   per-bar include chip strip; GetChecked() → a SNAPSHOT
                                      --   copy of { [abm|"pet"] = bool } (never the live table)
...
**`ns.BuildBarSelect(parent)`** builds a compact grid of toggle **chips** — one per action bar (`1‑8`, `C1‑C5`, `Bonus`, `Sky`) plus `Pet` — laid out in two rows. Each chip maps a short label to an abm bar number (`barlabels.lua` `BAR_DEFS`; the pet chip maps to the `"pet"` filter key). Chips **default to included** (gold wash + gold accent); clicking excludes (empty + red accent). `GetChecked()` returns `{ [abm | "pet"] = bool }` (explicit `true`/`false`; `false` = exclude) — consumed as the `barFilter` by Save (partial profiles) and Load; because all-`true` still counts 15 bars, `ns.Capture` treats "everything included" as a **full** profile. It hands back a **fresh snapshot**, never the live table: Load parks the filter on `f._pendingBarFilter` across the `ABM_CONFIRM_IMPORT` popup, behind which the chips stay clickable, and a click must not retroactively widen the restore the user already committed to (#132). `AllChecked()` / `SetAllChecked(v)` / `SetOnChanged(fn)` drive the window's **Check All / Uncheck All** button (header row, between the Profiles label and the class filter): the button's label is **derived** from `AllChecked()` and refreshed by the `SetOnChanged` ping, which fires after a chip click *and* after `SetAllChecked`. The button must not keep its own alternating flag — individual chip clicks desync it, and the label then offers an action that is already true (#132). `SetHighlighter(fn)` wires chip hover to the preview's `Highlight(abm, on)`, so hovering a chip lights up the bar it controls — the `Pet` chip has no preview row (`chip._abm` is nil), so it highlights nothing. Modeled on Warbandeer's `BarsApply` strip; the abm bar-number space is identical to LibNUI's `ui.BarsPreview` row keys (both `floor((slot-1)/12)+1`), so a chip's abm number *is* the preview bar it highlights.
...
- **Covered:** `libs/base64.lua`, `libs/crc32.lua`, `serialize.lua` (Encode/Decode round-trip, partial-profile v3, CRC tamper detection), `barlabels.lua`, `restore_slots.lua`'s `ns.RestoreSlots` petaction/futurespell clear (#131), and `barselect.lua`'s chip bookkeeping — defaults, per-chip toggling, `GetChecked` snapshot isolation, `AllChecked`, `SetAllChecked`, and the `SetOnChanged` aggregate (#132).
- **Loader:** `spec/abm.lua` `load(extra?, nui?)` loads those files into a fresh `ns` with the `(addonName, ns)` vararg, stubbing a pure-Lua **variadic** `bit` library (matches WoW's 4-arg `bor` that `Decode` relies on) and `IsWindowsClient`. No `luabitop` rock needed. `extra` loads further addon files on top — `restore_slots.lua` qualifies because every `C_*` reference in it is `and`-guarded, so nothing runs against the WoW API at load time.
- **Fake LibNUI:** `spec/abm.lua` `nui()` returns a thin widget double (`ui.Label` / `ui.Button` / `ui.Texture` with no-op chaining setters, `ui.edge` / `ui.layer` passthrough enums, `Colors.rgba`) installed as `ns.ui` / `ns.Colors` by `load(extra, nui)` **before** `extra` runs — `barselect.lua` captures `ns.Colors.rgba` at load time. Every Button is recorded in `nui.buttons` in creation order, so a spec drives `chips[i].OnClick()` the way the real widget's script does. Deliberately doubles only what `barselect.lua` touches; anything depending on real layout or rendering stays in-game-tested.
- **Fake action bar:** `spec/abm.lua` `actionbars(initial?)` stubs `GetActionInfo` / `PickupAction` / `PlaceAction` / `GetCursorInfo` / `ClearCursor` / `PickupSpell` over a slot table. Both pickup and place **swap** slot and cursor as they do in game — that swap is what turned "pick the action up to clear it" into a round-trip, so the harness must model it rather than treat pickup as a removal. Call it **before** `load()`: `restore_slots.lua` captures `PickupSpell` into a file-local upvalue at load time (the slot globals resolve per call, so only the load-time captures actually constrain the order).
